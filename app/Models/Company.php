...
class Company extends BaseModel
{
    ...
    const FIELDS_COMPANY = 'c1.id, 0 AS vrpid, 0 AS vrp_import, c1.active AS djs_active, c1.has_photo, c1.networks_active, c1.vendor_active, cs.id IS NOT NULL AS capabilies_active, "" AS vrp_primary_smff, "Vendor" AS vendor_category, 0 AS linked, 1 AS djs, 0 AS auth, c1.name AS name, 0 AS vrp_plan_name, c1.plan_number, c1.plan_number AS vrp_plan_number, cs.id IS NOT NULL AS resource_provider, c1.active, 0 AS vrp_status, c1.active AS coverage, cs.id AS smff_service_id, 0 AS vrp_plan_type, 0 AS vrp_country, qi_id, operating_company_id, email, fax, phone, website, description, c1.created_at, c1.updated_at';
    ...
    const FIELDS_ADDRESS = 'company_addresses.id AS id, c.id AS company_id, c.name AS text, latitude, longitude, smff_service_id';
