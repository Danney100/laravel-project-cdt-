...
class VesselController extends Controller
{
    ...
    public function getAll(Request $request)
    {
        ...
        if ($request->has('staticSearch')) {
            $filter = $this->staticSearch($cdtVessels, $request->get('staticSearch')); //unused variable
        }
        if (empty($vrp)) {
            ...
            $cdtVesselsOff = $cdtVesselsOff
                    ->leftJoin($vrpVesselTableName . " AS vrpv", function($join) {
                        ...
                        $join->on('c1.plan_number','=','vrpv.plan_number');
                    })
                    ->leftJoin($planTableName . " AS p", 'vrpv.plan_number_id','=','p.plan_number')
    ...
    public function importVrp($id)
    {
        ...
        return response()->json(['success' => false,'message' => 'This function will be available soon']);
            ...
            if($vessel){
                if($vessel->name != $vrpVessel->vessel_name){
                    $vessel->update([
                        ...
                        'company_id' => $vrpVessel->vrp_company_id > 0 ? $vrpVessel->vrp_company_id : $vessel->company_id ,
                ...
                Vessel::create([
                    ...
                    'imo' => $vrpVessel->imo,
                    'official_number' => $vrpVessel->official_number,
                    'company_id' => $vrpVessel->vrp_company_id,
    ...
    private function getVesselModal ($fleet_id=null, $model=null) {
        ...
        switch ($role_id) {
            case Role::COMPANY_PLAN_MANAGER : // Company Plan Manager
                return $model->whereIn('company_id', Auth::user()->companies()->pluck('id'));
            case Role::QI_COMPANIES : // QI Companies
                return $model->join('vessels_vendors AS vv', 'v1.id', '=', 'vv.vessel_id')
                        ->whereIn('vv.company_id', Company::where([['active', 1],['vendor_type', 3]])->pluck('id'));
            ...
            case Role::COAST_GUARD : // Coast Guard
                return $model->whereIn('company_id', Company::where('active', 1)->pluck('id'));
            case Role::NAVY_NASA :
...
//                    'response'   => $vessel->smff_service_id ? 1 : 0
        ...
        if($request->hasFile('file'))
        {
                ...
                if ($first < 3) {
                    ...
                    if (!empty(array_filter($row, function ($value) { return $value != ""; }))) {
                        ...
                        $to_be_added[] = [
                            ...
                            'company_id' => Company::where('name', $row[3])->first() ? Company::where('name', $row[3])->first()->id : 0,
                            'sat_phone_primary' => $row[4],
                            'sat_phone_secondary' => $row[5],
                            'email_primary' => $row[6],
                            'email_secondary' => $row[7],
                            'vessel_type_id' => VesselType::where('name', $row[8])->first() ? VesselType::where('name', $row[8])->first()->id : 0,
                            'vendor_type_1' => $row[9],
                            'vendor_type_2' => $row[10],
                            'dead_weight' => $row[11],
                            'vendor_type_3' => $row[12],
                            'vendor_type_4' => $row[13],
                            'vendor_type_5' => $row[14],
                            'deck_area' => $row[15],
                            'oil_tank_volume' => $row[16],
                            'oil_group' => $row[17],
                            'vendor_type_6' => $row[18],
                            'tanker' => $row[19] == "YES" ? 1 : 0,
                        ];
                    } else {
                        break;
                    }
            ...
            if (request('permitted')) {
                for ($i = 0; $i < count($to_be_added); $i++) {
                    ...
                    if($vessel->save()) {
                        if(Vendor::where('name', $to_be_added[$i]['vendor_type_1'])->first()) {
                            $vendorId1 = Vendor::where('name', $to_be_added[$i]['vendor_type_1'])->first()->id;
                            VesselVendor::create([
                                'vessel_id' => $vessel->id,
                                'vendor_id' => $vendorId1,
                            ]);
                        ...
                        if(Vendor::where('name', $to_be_added[$i]['vendor_type_2'])->first()) {
                            $vendorId2 = Vendor::where('name', $to_be_added[$i]['vendor_type_2'])->first()->id;
                            VesselVendor::create([
                                'vessel_id' => $vessel->id,
                                'vendor_id' => $vendorId2,
                            ]);
                        ...
                        if(Vendor::where('name', $to_be_added[$i]['vendor_type_3'])->first()) {
                            $vendorId3 = Vendor::where('name', $to_be_added[$i]['vendor_type_3'])->first()->id;
                            VesselVendor::create([
                                'vessel_id' => $vessel->id,
                                'vendor_id' => $vendorId3,
                            ]);
                        ...
                        if(Vendor::where('name', $to_be_added[$i]['vendor_type_4'])->first()) {
                            $vendorId4 = Vendor::where('name', $to_be_added[$i]['vendor_type_4'])->first()->id;
                            VesselVendor::create([
                                'vessel_id' => $vessel->id,
                                'vendor_id' => $vendorId4,
                            ]);
                        ...
                        if(Vendor::where('name', $to_be_added[$i]['vendor_type_5'])->first()) {
                            $vendorId5 = Vendor::where('name', $to_be_added[$i]['vendor_type_5'])->first()->id;
                            VesselVendor::create([
                                'vessel_id' => $vessel->id,
                                'vendor_id' => $vendorId5,
                            ]);
                        ...
                        if(Vendor::where('name', $to_be_added[$i]['vendor_type_6'])->first()) {
                            $vendorId6 = Vendor::where('name', $to_be_added[$i]['vendor_type_6'])->first()->id;
                            VesselVendor::create([
                                'vessel_id' => $vessel->id,
                                'vendor_id' => $vendorId6,
                            ]);
                        ...
                        return response()->json(['success'=> false, 'message' => 'Something unexpected happened.', 'error' => true, 'type' => 'vessel']);
                    }
                }
                return response()->json(['success'=> true, 'message' => $total . ' Vessels are added.', 'error' => false, 'type' => 'vessel']);
            } else if (count($dup_imos) || count($dup_officials)) {
                return response()->json(['success' => false, 'dup_imos' => $dup_imos, 'dup_officials' => $dup_officials, 'type' => 'vessel', 'error' => false]);
            } else
                return response()->json(['success'=> true, 'message' => $total . ' Vessels are added.', 'error' => false, 'type' => 'vessel']);
        }
        return response()->json(['success'=> false, 'message' => 'File not found.', 'error' => true, 'type' => 'vessel']);
    }
    // end store csv
    ...
    public function getLatestAISPositions(Request $request)
    {
        ...
        $page = $request['page'] ?? 1;
        $orderBy = $request['sort-by'] ?? 'name';
        $orderDir = $request['direction'] ?? 'asc';
