    ...
    public function getAIS_PS07_Bulk(Request $request)
    {
        ...
        if($vessel_count > 0) {
            foreach ($vessels as $vessel) {
                ...
                $imo = $vessel->imo;
                if($imo) {
                    // Get every vessel postions
                    $vesselInfo = VesselAISPositions::where('vessel_id', $id)->orderBy('updated_at', 'desc')->first();
                    if($vesselInfo) {
                        if($vesselInfo->updated_at <= request('last_update')) {
                            $aisResponse = MT_API_Helper::getAIS_PS07($id, $request->satellite, $request->extended);
                            if (!$aisResponse['success']) {
                                return response()->json($aisResponse);
                            }
                        }
                    } else {
                        $aisResponse = MT_API_Helper::getAIS_PS07($id, $request->satellite, $request->extended);
            ...
            if($request->network) {
                Network::where('id', $request->id)->update(['ais_last_updated_at'=> $nowTime]);
            } else {
                Fleet::where('id', $request->id)->update(['ais_last_updated_at' => $nowTime]);
            }
