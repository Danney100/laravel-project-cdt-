...
class MapController extends Controller
{
    ...
    public static function getFilteredCompanies($filters)
    {
        ...
        if (count($networks)) {
            ...
            //$addresses = $addresses->whereHas('company');
        }
        if (count($smff_selected)) {
            if ($smff_operator == "or") {
                ...
                $addresses->whereHas('company', function ($company) use ($smff_selected, $smff_operator) {
                    $count = 1;
                    foreach ($smff_selected as $service) {
                        $c = 'capabilities' . $count;
                        $cv = 'capabilities_values' . $count;
    ...
    public static function getFilteredIndividuals($filters)
    {
        $map_filters = json_decode($filters);
        $networks = $map_filters->networks;
        $smff_selected = $map_filters->smff_selected;
        ...
        $addresses = $userTable
                ->select(DB::raw("users.*, a.latitude, a.longitude, a.id AS address_id, us.primary_service"))
    ...
    public function searchMap($search)
    {
        ...
        if (count($geo) === 2 && $this->validateLatLong($geo[0], $geo[1])) {
            ...
            foreach ($vessels as $vessel) {
                if ($vessel->ais_lat && $vessel->ais_long && $vessel->ais_nav_status_id !== null) {
                    $imo = $vessel->imo ?? '--';
                    $data[] = [
                        ...
                        'info' => [
                            ...
                            'address_id' => 0,
                            'type' => 'vessel',
                            'latitude' => $vessel->ais_lat,
                            'longitude' => $vessel->ais_long
                        ]
                    ];
                }
            }
