    ...
    public static function getAIS_PS07 ($id, $satellite, $extended)
    {
        ...
        if($extended) {
            $aisPosition = VesselAISPositions::create([
                ...
                'shipname'  => $aisData->SHIPNAME,
                'shiptype'  => $aisData->SHIPTYPE,
                'type_name' => $aisData->TYPE_NAME,
                'ais_type_summary' => $aisData->AIS_TYPE_SUMMARY,
                'imo'       => $aisData->IMO,
                'callsign'  => $aisData->CALLSIGN,
                'flag'      => $aisData->FLAG,
                'port_id'   => $aisData->PORT_ID,
                'port_unlocode' => $aisData->PORT_UNLOCODE,
                'current_port'  => $aisData->CURRENT_PORT,
                'last_port_id'  => $aisData->LAST_PORT_ID,
                'last_port_unlocode' => $aisData->LAST_PORT_UNLOCODE,
                'last_port' => $aisData->LAST_PORT,
                'last_port_time' => $aisData->LAST_PORT_TIME,
                'destination' => $aisData->DESTINATION,
                'eta'       => $aisData->ETA,
                'eta_calc'  => $aisData->ETA_CALC,
                'length'    => $aisData->LENGTH,
                'width'     => $aisData->WIDTH,
                'draught'   => $aisData->DRAUGHT,
                'grt'       => $aisData->GRT,
                'dwt'       => $aisData->DWT,
                'year_built'       => $aisData->YEAR_BUILT,
                'next_port_id' => $aisData->NEXT_PORT_ID,
                'next_port_unlocode' => $aisData->NEXT_PORT_UNLOCODE,
                'next_port_name' => $aisData->NEXT_PORT_NAME,
                'msgtype'   => 1,
                'dsrc'      => $aisData->DSRC,
                'zone_id'   => $zone_id,
            ]);
